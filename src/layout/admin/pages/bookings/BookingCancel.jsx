...
const BookingCancel = () => {
    ...
    const fetchBookings = async () => {
        try {
            ...
            const response = await fetch('http://localhost:8000/api/v1/booking/cancelled/', {
                method: 'GET',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            });

            if (response.status === 401 || response.status === 403) { navigate('/admin-login'); return; }
            if (!response.ok) throw new Error(`Failed to load cancelled bookings (HTTP ${response.status})`);
        ...
        <AdminLayout>
            <div className="p-4">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold text-red-700 dark:text-red-400">Cancelled Bookings</h1>
                    <button onClick={fetchBookings} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded text-sm">
                        Refresh
                    </button>
                </div>
                ...
                {bookings.length === 0 ? (
                    ...
                    <>
                        <p className="text-sm text-gray-500 dark:text-stone-400 mb-4">
                            Total: {bookings.length} cancelled booking{bookings.length > 1 ? 's' : ''}
                        </p>
                                        ...
                                        return (
                                            <tr key={b.id} className="hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-sm">
                                                    <span className="px-2 py-1 bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300 text-xs font-semibold rounded-full">
                                                        Cancelled
                                                    </span>
                                                </td>
                                            </tr>
